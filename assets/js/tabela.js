...
(function (global) {
  ...
  var COLUNAS = [
    ...
    { id: 'caminho', titulo: 'Caminho do waiver', larg: 32, valor: function (r) { return caminhoDe(r); } },
  ...
  /**
   * O caminho inteiro: por onde passou **e** para onde vai.
   *
   * `Fluxo.caminhoTexto` só sabe do passado — o Waiver Historic de um item do
   * J08 termina em J08. O destino vem do documento (`Herdar.avanco`), e vai
   * entre parênteses enquanto o item não tiver sido levado para lá:
   *
   *     J04 → J06 → J08 → (J09)    ainda não levada
   *     J06 → J08 → J09            a cópia já está no J09
   *
   * O parêntese é o que separa "já aconteceu" de "vai acontecer". Sem ele,
   * quem lê a planilha entende que a NCR já está no J09.
   */
  function caminhoDe(r) {
    if (r.caminhoTxt == null) {
      var base = Fluxo.caminhoTexto(fluxoDe(r));
      var a = avancoDe(r);
      if (a.destino && !passaPelo(fluxoDe(r), a.destino.chave)) {
        var alvo = '(' + a.destino.rotulo + ')';
        base = base ? base + ' \u2192 ' + alvo : alvo;
      }
      r.caminhoTxt = base;
    }
    return r.caminhoTxt;
  ...
  /** O desenho do fluxo já tem este marco? */
  function passaPelo(analise, chave) {
    if (!analise || analise.vazio) return false;
    return (analise.nos || []).some(function (n) { return n.chave === chave; });
  }
  ...
  /* `marcos` é uma lista de ids de relatório; vazia = todos. Era um campo só
     (`marco`), de escolha única, e comparar dois marcos exigia trocar de
     filtro e voltar. Filtro guardado de versão antiga com `marco` preenchido
     continua valendo — `marcosDoFiltro` entende os dois. */
  var VAZIO = { marcos: [], tipo: '', situacao: '', sistema: '', archStatus: '',
                evidencia: '', busca: '', parados: false, comNota: false };

  function marcosDoFiltro(f) {
    if (!f) return [];
    if (f.marcos && f.marcos.length) return f.marcos;
    return txt(f.marco) ? [txt(f.marco)] : [];
  }

  function algumFiltro(f) {
    if (!f) return false;
    return !!(marcosDoFiltro(f).length || txt(f.tipo) || txt(f.situacao) || txt(f.sistema) ||
      txt(f.archStatus) || txt(f.evidencia) || txt(f.busca) || f.parados || f.comNota);
  }

  function passa(r, f) {
    if (!f) return true;
    var ms = marcosDoFiltro(f);
    if (ms.length && ms.indexOf(r.projetoId) < 0) return false;
  ...
  function render(host, projects, estado, acoes) {
    ...
    /* --- filtros --- */
    /* Os marcos em pastilhas, numa linha própria: são a escolha mais usada e
       a única em que faz sentido marcar várias ("o J08 e o J09 juntos"). As
       mesmas pastilhas do filtro de situação do PDF e do "Passa por" da aba
       Fluxos — a mesma ideia tem de ter a mesma cara no programa inteiro. */
    ...
    (projects || []).forEach(function (p) {
      ...
      var b = el('button', 'tb-chip' + (ligado ? ' is-on' : ''),
        p.marco || p.name || 'sem marco');
      b.type = 'button';
      b.setAttribute('aria-pressed', ligado ? 'true' : 'false');
      b.title = (ligado ? 'Tirar ' : 'Acrescentar ') + (p.marco || p.name) +
        ' (' + n + ' item(ns)). Dá para marcar vários.';
      b.appendChild(el('span', 'tb-chip-n', String(n)));
      b.addEventListener('click', function () {
        /* `marco` (escolha única, das versões anteriores) sai de cena assim
           que alguém mexe aqui, senão os dois filtrariam ao mesmo tempo */
        f.marco = '';
        f.marcos = ligado
          ? escolhidos.filter(function (id) { return id !== p.id; })
          : escolhidos.concat([p.id]);
        mudou(false);
      });
      chips.appendChild(b);
    });
    ...
    if ((projects || []).length > 1) host.appendChild(chips);
  ...
  function descricaoDoFiltro(f, projects) {
    ...
    var ms = marcosDoFiltro(f);
    if (ms.length) {
      var nomes = ms.map(function (id) {
        var p = (projects || []).filter(function (x) { return x.id === id; })[0];
        return p ? (p.marco || p.name || 'sem marco') : id;
      });
      ditos.push((nomes.length === 1 ? 'marco: ' : 'marcos: ') + nomes.join(', '));
    }
